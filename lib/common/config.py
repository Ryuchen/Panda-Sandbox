...
from lib.defines.types import Int, Boolean, String, Path, List
from lib.exceptions.critical import CuckooConfigurationError
...
class Config(object):
    ...
    configuration = {
        "cuckoo": {
            "cuckoo": {
                ...
                "freespace": Int(1024),
                "tmppath": Path(
                    exists=True, writable=True, readable=False, allow_empty=True
                ),
                "api_token": String(allow_empty=True, sanitize=True, required=False),
                "web_secret": String(allow_empty=True, sanitize=True, required=False),
                "rooter": Path(
                    "/tmp/cuckoo-rooter", exists=False, writable=False, readable=False
                ),
            },
            "feedback": {
            ...
            "timeouts": {"default": Int(120), "critical": Int(60), "vm_state": Int(60)},
            "remotecontrol": {
                "enabled": Boolean(False),
        ...
        "virtualbox": {
            "virtualbox": {
                ...
                "path": Path(
                    "/usr/bin/VBoxManage", exists=False, writable=False, readable=True
                ),
        ...
        "auxiliary": {
            "sniffer": {
                ...
                "tcpdump": Path(
                    "/usr/sbin/tcpdump", exists=False, writable=False, readable=True
                ),
                "bpf": String(),
            },
            "mitm": {
                ...
                "port_base": Int(50000),
                "script": Path(
                    "stuff/mitm.py", exists=False, writable=False, readable=True
                ),
                "certificate": Path(
                    "bin/cert.p12", exists=False, writable=False, readable=True
                ),
            },
            "replay": {
        ...
        "memory": {
            ...
            "malfind": {"enabled": Boolean(True), "filter": Boolean(True)},
            "apihooks": {"enabled": Boolean(False), "filter": Boolean(True)},
            "pslist": {"enabled": Boolean(True), "filter": Boolean(False)},
            "psxview": {"enabled": Boolean(True), "filter": Boolean(False)},
            "callbacks": {"enabled": Boolean(True), "filter": Boolean(False)},
            "idt": {"enabled": Boolean(True), "filter": Boolean(False)},
            "timers": {"enabled": Boolean(True), "filter": Boolean(False)},
            "messagehooks": {"enabled": Boolean(False), "filter": Boolean(False)},
            "getsids": {"enabled": Boolean(True), "filter": Boolean(False)},
            "privs": {"enabled": Boolean(True), "filter": Boolean(False)},
            "dlllist": {"enabled": Boolean(True), "filter": Boolean(True)},
            "handles": {"enabled": Boolean(True), "filter": Boolean(True)},
            "ldrmodules": {"enabled": Boolean(True), "filter": Boolean(True)},
            "mutantscan": {"enabled": Boolean(True), "filter": Boolean(True)},
            "devicetree": {"enabled": Boolean(True), "filter": Boolean(True)},
            "svcscan": {"enabled": Boolean(True), "filter": Boolean(True)},
            "modscan": {"enabled": Boolean(True), "filter": Boolean(True)},
            "yarascan": {"enabled": Boolean(True), "filter": Boolean(True)},
            "ssdt": {"enabled": Boolean(True), "filter": Boolean(True)},
            "gdt": {"enabled": Boolean(True), "filter": Boolean(True)},
            "sockscan": {"enabled": Boolean(True), "filter": Boolean(False)},
            "netscan": {"enabled": Boolean(True), "filter": Boolean(False)},
            "mask": {"enabled": Boolean(False), "pid_generic": List(String, None)},
        },
        "processing": {
            "analysisinfo": {"enabled": Boolean(True)},
            "apkinfo": {
                "enabled": Boolean(False),
                "decompilation_threshold": Int(5000000),
            },
            "baseline": {"enabled": Boolean(False)},
            "behavior": {"enabled": Boolean(True)},
            "buffer": {"enabled": Boolean(True)},
            "debug": {"enabled": Boolean(True)},
            "droidmon": {"enabled": Boolean(False)},
            "dropped": {"enabled": Boolean(True)},
            "dumptls": {"enabled": Boolean(True)},
            "extracted": {"enabled": Boolean(True, required=False)},
            "googleplay": {
                "enabled": Boolean(False),
                "android_id": String(),
                "google_login": String(),
            ...
            "procmon": {"enabled": Boolean(True)},
            "screenshots": {"enabled": Boolean(True), "tesseract": String("no")},
            "snort": {
                "enabled": Boolean(False),
                "snort": Path(
                    "/usr/local/bin/snort", exists=False, writable=False, readable=True
                ),
                "conf": Path(
                    "/etc/snort/snort.conf", exists=False, writable=False, readable=True
                ),
            },
            "static": {"enabled": Boolean(True), "pdf_timeout": Int(60)},
            "strings": {"enabled": Boolean(True)},
            "suricata": {
                "enabled": Boolean(False),
                "suricata": Path(
                    "/usr/bin/suricata", exists=True, writable=False, readable=True
                ),
                "conf": Path(
                ...
                "eve_log": Path(
                    "eve.json", exists=False, writable=True, readable=False
                ),
                "files_log": Path(
                    "files-json.log", exists=False, writable=True, readable=False
                ),
                "files_dir": Path("files", exists=False, writable=False, readable=True),
                "socket": Path(
                    exists=True, writable=False, readable=True, allow_empty=True
                ),
            },
            "targetinfo": {"enabled": Boolean(True)},
            "virustotal": {
                "enabled": Boolean(False),
                "timeout": Int(60),
                "scan": Boolean(False),
        ...
        "routing": {
            ...
            "vpn": {"enabled": Boolean(False), "vpns": List(String, "vpn0")},
            "*": {
                "__section__": "vpn0",
                "name": String("vpn0"),
    ...
    @staticmethod
    def get_section_types(file_name, section, strict=False, loose=False):
    ...
    def __init__(self, file_name="cuckoo", cfg=None, strict=False, loose=False, raw=False):
        """
        ...
        try:
            ...
            raise CuckooConfigurationError(
                ...
                "More information from the original exception: %s" % (file_name, e)
            )
        ...
        for section in config.sections():
            ...
            try:
                ...
                raise CuckooConfigurationError("Missing environment variable: %s" % e)
    ...
    def from_confdir(dirpath, loose=False, sanitize=False):
        ...
        for filename in os.listdir(dirpath):
            ...
            cfg = Config(config_name, cfg=os.path.join(dirpath, filename), loose=loose)

            ret[config_name] = {}
            for section, values in cfg.sections.items():
                ret[config_name][section] = {}
                types = cfg.get_section_types(config_name, section, loose=loose) or {}
...
def config(s, cfg=None, strict=False, raw=False, loose=False, check=False):
    ...
    if strict and type_ is None:
        raise CuckooConfigurationError("No such configuration value exists: %s" % s)

    required = type_ is not None and type_.required
    index = file_name, cfg, cwd(), strict, raw, loose

    if index not in _cache:
        _cache[index] = Config(file_name, cfg=cfg, strict=strict, raw=raw, loose=loose)
...
def config2(file_name, section):
    ...
    if not keys:
        raise CuckooConfigurationError(
            "No such configuration section exists: %s:%s" % (file_name, section)
        )
...
def read_kv_conf(filepath):
    ...
    for line in open(filepath, "rb"):
        ...
        if "=" not in line:
            raise CuckooConfigurationError(
                "Invalid flat configuration line: %s (missing '=' character)" % line
            )
